...
const SpotListItem: React.FC<SpotListItemProps> = ({ spot, onClick, onTogglePin }) => {
  ...
  return (
    <div
      className="group relative bg-white dark:bg-neutral-800 rounded-3xl overflow-hidden cursor-pointer transition-all duration-500 hover:shadow-lg border border-neutral-100 dark:border-neutral-700/50 flex min-h-[8rem]"
      onClick={onClick}
    >
      {/* Image Section - Fixed Width, Grow Height */}
      <div className="relative w-36 flex-shrink-0 self-stretch">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-black/20 to-transparent opacity-50"></div>
         ...
         <button 
          ...
          className={`absolute top-2 right-2 w-7 h-7 rounded-full flex items-center justify-center border shadow-sm z-10 transition-all duration-200 hover:scale-110 active:scale-90 ${spot.isPinned ? 'bg-rose-500 border-rose-500 text-white' : 'bg-white/30 backdrop-blur-md border-white/20 text-white hover:bg-white/40'}`}
        >
             <Icon name="heart" className={`w-3.5 h-3.5 ${spot.isPinned ? 'fill-white' : ''}`} />
        </button>

        {/* Status Badge - Moved to Image Overlay */}
        <div className={`absolute top-2 left-2 inline-flex items-center gap-1 pl-2 pr-2.5 py-0.5 rounded-full text-[10px] font-bold ${badge} ${text} shadow-sm z-10 backdrop-blur-md`}>
      ...
      <div className="flex-1 flex flex-col p-3.5 min-w-0">
        <div className="flex-1">
            <div className="flex justify-between items-start gap-2 mb-1">
                 {/* Rating */}
                 {spot.rating && (
                    <div className="flex items-center gap-1 bg-amber-50 dark:bg-amber-900/30 px-1.5 py-0.5 rounded-md ml-auto">
                 ...
                 {!spot.rating && <div className="h-5"></div>} {/* Spacer to align title if no rating */}
            </div>

            <h3 className="font-bold text-[15px] text-neutral-800 dark:text-neutral-100 leading-snug line-clamp-3 group-hover:text-rose-500 transition-colors mb-2">
                {spot.name}
            </h3>
        </div>
        ...
        <div className="flex flex-col gap-1.5 mt-auto">
            ...
            <div className="flex items-center gap-3 text-[11px] text-neutral-400 font-medium border-t border-neutral-100 dark:border-neutral-700/50 pt-1.5 mt-0.5">
                 <span className="flex-shrink-0">{(spot.visitCount || 0) > 0 ? `${spot.visitCount}回訪問` : '未訪問'}</span>
                 {spot.lastVisitDate && (
                     <span className="truncate">最終: {new Date(spot.lastVisitDate).toLocaleDateString()}</span>
                 )}
            </div>
        </div>
      </div>
